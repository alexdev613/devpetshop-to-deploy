...
export function Header() {
  ...
  function getLinks() {
    ...
    if (matchCart) {
      return (
        <>
          ...
          <Link to='/favoritos' className="hover:bg-slate-400">
            <h3>FAVORITOS</h3>
          </Link>
        </>
      );
    ...
    if (matchFavorites) {
      return (
        <>
          ...
          <Link to='/carrinho' className="hover:bg-slate-400">
            <h3>CARRINHO</h3>
          </Link>
        </>
      );
    ...
    // retrun null; garante que a função getLinks sempre retorna um valor, mesmo que nunhuma das consdições anteriores seja atendida
    return null;
